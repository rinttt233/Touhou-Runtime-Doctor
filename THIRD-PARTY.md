...
本仓库**不包含任何第三方二进制文件**——运行库、DirectX DLL、第三方工具包都不在仓库里。

仓库里有一个抓取脚本 `tools/Fetch-OfflinePack.ps1`，它只负责从**官方渠道**
（微软下载中心、各上游项目的 GitHub Release）把文件下载到本地 `offline\` 目录。
它本身不携带、不重新分发任何第三方组件；仓库里放的是"下载这些库的脚本"，不是库本体。
